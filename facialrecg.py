...
class FaceRecog:
	...
	def __init__(self):
		...
		try:
			...
			with open (r'C:\Users\anjan\Documents\Programming\Js\labels.pickle','rb') as self.f:
		...
		if self.labels_ids != self.og_labels:
			...
			with open (r'C:\Users\anjan\Documents\Programming\Js\KnownFace.pickle','rb') as self.faces_file:
				self.known_faces = pickle.load(self.faces_file)
			print(self.known_faces)
	#ID Function
	#This will be used to create a live feed by turning on the camera
	#then the life feed is used to get an image and then encode the users face
	#once its encoded it ill be used be compared to the known faces and identify the user
...
def register():
	...
	#create folder of person (if it does not exist) in the images folder

	Path("images/" + name.get()).mkdir(parents = True, exist_ok = True) #use name varible from tinkter

	#Obtain the number of photos already in the folder
	numberOfFile = numberOfFile = len([filename for filename in os.listdir('images/' + name.get())
						if os.path.isfile(os.path.join('images/' + name.get(), filename))])

	#Add 1 because we are adding one more or start at 1
	...
	while True:
		...
		if k % 256 == 27:
			#ESCAPE PRESSED
			print("Escape enterd, camera is closing...")
			cam.release()
			cv2.destroyAllWindows()
			break
		elif k % 256 == 32:
...
#Passing our ID model
def login():
	#After registration, the face scanner ID will be called and new face will be created
	facelogin = FaceRecog()

	#will need return the user's name as a list, will return an empty list if nothing matches

	user = facelogin.ID()

	if user == []:
		messagebox.showerror("Alert", "Face Not Recognized")
		return
...
root = tk.Tk()
root.title("FACE LOGIN PROGRAM DEMO")
...
button = tk.Button(loginFrame, text = "Click and Quit", command = close_window, font=("Arial", 20)).grid(row=2, column=6)
